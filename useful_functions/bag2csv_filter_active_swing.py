...
csvDir = workspace_path + '/Converted_CSV_Data_Collection_5/'
...
W = 75
data_load = False
NUM_OBSERVATIONS_PRE_ES1 = 0
NUM_OBSERVATIONS_POST_ES2 = 0
...
for b in range(0, len(bagfilelist)):
    ...
    #3. Collect Early Stances and Swing Flexions
    FSM_SF_idx = []
    FSM_ES_idx = []
    for f in range(0, len(FSM_states)):
        ...
        if 'SwingFlexion' in FSM_states[f]:
            FSM_SF_idx.append(f)

    #4. Go Through Pairs of Early Stances
    # min(len(FSM_SF_idx),len(FSM_SF_idx))
    if len(FSM_ES_idx) > len(FSM_SF_idx):
        FSM_ES_idx = FSM_ES_idx[1:]

    for ff in range(1, len(FSM_ES_idx) - 1):
        
        # ambulation mode always level walking for us
        ...
        # Get the Previous SF as well as the current and next one
        prev_sf_time = FSM_headers[FSM_SF_idx[ff - 1]]
        sf_1_time = FSM_headers[FSM_SF_idx[ff]]
        sf_2_time = FSM_headers[FSM_SF_idx[ff + 1]]

        # ES Get the corresponding indices based on timestamp
        ...
        # SF Get the corresponding indices based on timestamp
        prev_idx_SensorData = al.closest_point(loaded_bag['/SensorData']['header'], prev_sf_time)
        sf_1_idx_SensorData = al.closest_point(loaded_bag['/SensorData']['header'], sf_1_time)
        sf_2_idx_SensorData = al.closest_point(loaded_bag['/SensorData']['header'], sf_2_time)
        ...
        for key in loaded_bag['/SensorData'].keys():
            csv_dict[key] = loaded_bag['/SensorData'][key][
                             sf_1_idx_SensorData - NUM_OBSERVATIONS_PRE_ES1:es_1_idx_SensorData + NUM_OBSERVATIONS_POST_ES2]
